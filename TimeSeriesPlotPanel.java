...
public final class TimeSeriesPlotPanel extends JPanel {
    ...
    public void setSeries(long[] tEpochMillis, double[] y, boolean[] anomaly, String[] event, String yLabel) {
    ...
    protected void paintComponent(Graphics g) {
        ...
        try {
            ...
            if (tEpochMillis.length == 0 || y.length == 0) {
                g2.setColor(new Color(90, 90, 90));
                g2.setFont(new Font("Arial", Font.PLAIN, 12));
                g2.drawString("No time-series data.", 12, 20);
                return;
            }
            ...
            for (int i = 0; i <= xTicks; i++) {
                int xx = left + (int) Math.round((i / (double) xTicks) * pw);
                g2.drawLine(xx, top, xx, top + ph);
            }
            int yTicks = 6;
            for (int i = 0; i <= yTicks; i++) {
                int yy = top + ph - (int) Math.round((i / (double) yTicks) * ph);
                g2.drawLine(left, yy, left + pw, yy);
            }
            ...
            g2.setColor(Color.DARK_GRAY);
            g2.drawString(yLabel, 8, top + fm.getAscent());
            ...
            for (int i = 0; i < y.length; i++) {
                ...
                int yy = top + ph - (int) Math.round(((v - yMin) / (yMax - yMin)) * ph);
                if (prevX >= 0) {
            ...
            drawMarker(g2, left, top, pw, ph, yMin, yMax, selectedIndex, new Color(20, 184, 166));
            drawMarker(g2, left, top, pw, ph, yMin, yMax, hoverIndex, new Color(148, 163, 184));

            if (hoverIndex >= 0 && hoverIndex < tEpochMillis.length && hoverIndex < y.length) {
                long t = tEpochMillis[hoverIndex];
                double v = y[hoverIndex];
                if (t >= viewStart && t <= viewEnd && Double.isFinite(v)) {
                    String label = TS.format(Instant.ofEpochMilli(t)) + "  " + yLabel + "=" + String.format("%.6f", v);
    ...
    private void drawMarker(Graphics2D g2, int left, int top, int pw, int ph, double yMin, double yMax, int idx, Color c) {
        if (idx < 0 || idx >= tEpochMillis.length || idx >= y.length) {
            return;
        }
        ...
        int yy = top + ph - (int) Math.round(((v - yMin) / (yMax - yMin)) * ph);
        g2.setColor(c);
        g2.fillOval(x - 4, yy - 4, 8, 8);
        g2.setColor(Color.WHITE);
        g2.drawOval(x - 4, yy - 4, 8, 8);
    }
